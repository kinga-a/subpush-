...
function formatBeijingTime(date = new Date(), format = 'full') {
  ...
  <body class="login-container flex items-center justify-center">
    ...
    <div class="login-box p-10 rounded-3xl w-full max-w-md mx-4">
      <div class="text-center mb-10">
        <div class="logo-icon">
          <i class="fas fa-calendar-check"></i>
        </div>
        <h1 class="text-3xl font-bold gradient-text mb-3">订阅管理系统</h1>
        <p class="text-gray-600 text-lg">智能管理您的订阅提醒</p>
      ...
      <form id="loginForm" class="space-y-6">
        <div class="space-y-2">
          ...
          <input type="text" id="username" name="username" required
            class="input-field w-full px-5 py-4 rounded-xl text-gray-700 focus:outline-none text-lg"
            placeholder="请输入用户名">
        </div>
        ...
        <button type="submit" 
          class="btn-primary w-full py-4 rounded-xl text-white font-semibold text-lg focus:outline-none mt-8">
          <i class="fas fa-sign-in-alt mr-3"></i>登录系统
        </button>
  ...
  <body class="bg-gray-100 min-h-screen">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
          <div class="flex items-center">
            ...
            <span class="font-bold text-2xl gradient-text">订阅管理系统</span>
          </div>
          <div class="flex items-center space-x-2">
            ...
            <a href="/api/logout" class="nav-link text-red-600 hover:bg-red-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="stats-card fade-in-up" style="animation-delay: 0.1s;">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">总订阅数</p>
              <p class="text-2xl font-bold" id="totalSubscriptions">-</p>
            </div>
        ...
        <div class="stats-card fade-in-up" style="animation-delay: 0.2s;">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">即将到期</p>
              <p class="text-2xl font-bold" id="expiringSoon">-</p>
            </div>
            <i class="fas fa-exclamation-triangle text-3xl text-blue-100"></i>
          </div>
        </div>
        ...
        <div class="stats-card fade-in-up" style="animation-delay: 0.3s;">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">已过期</p>
              <p class="text-2xl font-bold" id="expiredCount">-</p>
            </div>
            <i class="fas fa-times-circle text-3xl text-blue-100"></i>
          </div>
        </div>
        ...
        <div class="stats-card fade-in-up" style="animation-delay: 0.4s;">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">活跃订阅</p>
              <p class="text-2xl font-bold" id="activeSubscriptions">-</p>
            </div>
            <i class="fas fa-check-circle text-3xl text-blue-100"></i>
          </div>
        </div>
      </div>
      
      <div class="main-container p-8 fade-in-up" style="animation-delay: 0.5s;">
        <div class="flex justify-between items-center mb-8">
          <div>
            <h2 class="text-3xl font-bold gradient-text mb-2">订阅列表</h2>
            <p class="text-gray-600">管理您的所有订阅提醒</p>
          </div>
          <div class="flex items-center space-x-4">
            <label class="lunar-toggle">
              <input type="checkbox" id="listShowLunar" class="form-checkbox h-4 w-4 text-indigo-600">
              <span class="text-gray-700 font-medium">显示农历</span>
            </label>
            <button id="addSubscriptionBtn" class="btn-primary text-white px-6 py-3 rounded-xl text-sm font-semibold flex items-center">
      ...
      <!-- 浮动操作按钮 -->
      <div class="floating-action-btn" id="floatingAddBtn" title="快速添加订阅">
  ...
  function addLunarPeriod(lunar, periodValue, periodUnit) {
    ...
    return { year, month, day, isLeap };
      ...
      function updateStats(data) {
        ...
        const active = data.filter(s => s.isActive !== false).length;
        const expired = data.filter(s => {
          if (s.isActive === false) return false;
          const daysDiff = Math.ceil((new Date(s.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
          return daysDiff < 0;
        }).length;
        const expiringSoon = data.filter(s => {
          if (s.isActive === false) return false;
          const daysDiff = Math.ceil((new Date(s.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
          return daysDiff >= 0 && daysDiff <= (s.reminderDays || 7);
        }).length;
      ...
      async function loadSubscriptions() {
        try {
          ...
          data.forEach(subscription => {
            ...
            row.className = subscription.isActive === false ? 'table-row hover:bg-gray-50 bg-gray-100' : 'table-row hover:bg-gray-50';
            ...
            if (!subscription.isActive) {
              statusHtml = '<span class="status-badge bg-gray-500 text-white"><i class="fas fa-pause-circle mr-1"></i>已停用</span>';
            } else if (daysDiff < 0) {
              statusHtml = '<span class="status-badge bg-red-500 text-white"><i class="fas fa-exclamation-circle mr-1"></i>已过期</span>';
              ...
              statusHtml = '<span class="status-badge bg-green-500 text-white"><i class="fas fa-check-circle mr-1"></i>正常</span>';
            }
  ...
  <head>
    ...
    <style>
      * {
      ...
      .toast {
        position: fixed; top: 20px; right: 20px; padding: 16px 24px; border-radius: 16px;
  ...
  <body class="min-h-screen">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
          ...
          <div class="flex items-center space-x-2">
            ...
            <a href="/admin/config" class="nav-link active">
              <i class="fas fa-cog mr-2"></i>系统配置
            </a>
            <a href="/api/logout" class="nav-link text-red-600 hover:bg-red-50">
              <i class="fas fa-sign-out-alt mr-2"></i>退出登录
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="main-container p-8 fade-in-up">
        <div class="text-center mb-8">
          <h2 class="text-4xl font-bold gradient-text mb-4">系统配置</h2>
          <p class="text-gray-600 text-lg">配置系统参数和通知设置</p>
        ...
        <form id="configForm" class="space-y-8">
          <div class="section-header">
            <h3 class="text-xl font-semibold text-gray-900 mb-4 flex items-center">
              <i class="fas fa-user-shield mr-3 text-blue-600"></i>管理员账户
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="adminUsername" class="block text-sm font-semibold text-gray-700 mb-2">用户名</label>
                <input type="text" id="adminUsername" class="form-input w-full">
              </div>
              <div>
                <label for="adminPassword" class="block text-sm font-semibold text-gray-700 mb-2">密码</label>
                <input type="password" id="adminPassword" placeholder="如不修改密码，请留空" class="form-input w-full">
          ...
          <div class="section-header">
            <h3 class="text-xl font-semibold text-gray-900 mb-4 flex items-center">
              <i class="fas fa-eye mr-3 text-green-600"></i>显示设置
            </h3>
            <div class="mb-6">
              <label class="inline-flex items-center p-4 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                <input type="checkbox" id="showLunarGlobal" class="form-checkbox h-5 w-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" checked>
                <span class="ml-3 text-sm font-medium text-gray-700">在通知中显示农历日期</span>
              </label>
          ...
          <div class="section-header">
            <h3 class="text-xl font-semibold text-gray-900 mb-4 flex items-center">
              <i class="fas fa-bell mr-3 text-yellow-600"></i>通知设置
            </h3>
            <div class="mb-6">
              <label class="block text-sm font-semibold text-gray-700 mb-4">通知方式（可多选）</label>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">Telegram</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700 font-semibold">NotifyX</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">企业微信应用通知</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-sm font-medium text-gray-700">企业微信机器人</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">邮件通知</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">钉钉通知</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">飞书通知</span>
                </label>
                <label class="inline-flex items-center p-3 bg-white rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-300 cursor-pointer">
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-700">Bark 推送</span>
                </label>
              </div>
              <div class="mt-4 flex flex-wrap gap-4">
                <a href="https://www.notifyx.cn/" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm bg-white px-3 py-2 rounded-lg border border-gray-200 hover:border-indigo-300 transition-all duration-300">
                  <i class="fas fa-external-link-alt mr-2"></i> NotifyX官网
                </a>
                <a href="https://push.wangwangit.com" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm bg-white px-3 py-2 rounded-lg border border-gray-200 hover:border-indigo-300 transition-all duration-300">
                  <i class="fas fa-external-link-alt mr-2"></i> 企业微信应用通知官网
                </a>
                <a href="https://developer.work.weixin.qq.com/document/path/91770" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm bg-white px-3 py-2 rounded-lg border border-gray-200 hover:border-indigo-300 transition-all duration-300">
                  <i class="fas fa-external-link-alt mr-2"></i> 企业微信机器人文档
                </a>
                <a href="https://developers.cloudflare.com/workers/tutorials/send-emails-with-resend/" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm bg-white px-3 py-2 rounded-lg border border-gray-300 hover:border-indigo-300 transition-all duration-300">
                  <i class="fas fa-external-link-alt mr-2"></i> 获取 Resend API Key
                </a>
                <a href="https://github.com/Finb/Bark" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm bg-white px-3 py-2 rounded-lg border border-gray-200 hover:border-indigo-300 transition-all duration-300">
                  <i class="fas fa-external-link-alt mr-2"></i> Bark 官网
                </a>
              </div>
            </div>
            ...
            <div id="telegramConfig" class="config-section">
              <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <i class="fab fa-telegram mr-3 text-blue-500"></i>Telegram 配置
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label for="tgBotToken" class="block text-sm font-medium text-gray-700">Bot Token</label>
                  <input type="text" id="tgBotToken" placeholder="从 @BotFather 获取" class="form-input w-full">
                </div>
                <div>
                  <label for="tgChatId" class="block text-sm font-medium text-gray-700">Chat ID</label>
                  <input type="text" id="tgChatId" placeholder="可从 @userinfobot 获取" class="form-input w-full">
                </div>
              </div>
              <div class="flex justify-end">
                <button type="button" id="testTelegramBtn" class="btn-secondary text-white px-4 py-2 rounded-xl text-sm font-medium">
            ...
            <div id="notifyxConfig" class="config-section">
              <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <i class="fas fa-bell mr-3 text-green-500"></i>NotifyX 配置
              </h4>
              <div class="mb-4">
                <label for="notifyxApiKey" class="block text-sm font-medium text-gray-700">API Key</label>
                <input type="text" id="notifyxApiKey" placeholder="从 NotifyX 平台获取的 API Key" class="form-input w-full">
              ...
              <div class="flex justify-end">
                <button type="button" id="testNotifyXBtn" class="btn-secondary text-white px-4 py-2 rounded-xl text-sm font-medium">
            ...
            <div id="webhookConfig" class="config-section">
              <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <i class="fab fa-weixin mr-3 text-green-600"></i>企业微信应用通知 配置
              </h4>
            ...
            <div id="barkConfig" class="config-section">
              <h4 class="text-md font-medium text-gray-900 mb-3">Bark 推送 配置</h4>
              <div class="grid grid-cols-1 gap-4 mb-4">
                <div>
                  <label for="barkServer" class="block text-sm font-medium text-gray-700">Bark 服务器地址</label>
                  ...
                  <p class="mt-1 text-sm text-gray-500">Bark 推送服务器地址，默认为官方服务器 https://api.day.app</p>
                </div>
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500">从 Bark 应用获取的设备推送 Key</p>
                </div>
                <div>
                  <label for="barkTitle" class="block text-sm font-medium text-gray-700">消息标题模板</label>
                  <input type="text" id="barkTitle" placeholder="{{title}}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  <p class="mt-1 text-sm text-gray-500">支持变量: {{title}} 为通知标题，{{subscription}} 为订阅名称</p>
                </div>
                <div>
                  <label for="barkBody" class="block text-sm font-medium text-gray-700">消息内容模板</label>
                  <textarea id="barkBody" rows="3" placeholder="{{content}}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                  <p class="mt-1 text-sm text-gray-500">支持变量: {{content}} 为通知内容，{{subscription}} 为订阅名称，{{expiryDate}} 为到期日期</p>
                </div>
                <div>
                  <label for="barkSound" class="block text-sm font-medium text-gray-700">推送音效</label>
                  <select id="barkSound" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <option value="anticipate">期待音</option>
                    <option value="bell">铃声</option>
                    <option value="birdsong">鸟鸣声</option>
                    <option value="bloom">绽放音</option>
                    <option value="calypso">卡利普索</option>
                    <option value="chime">风铃声</option>
                    <option value="choo">火车声</option>
                    <option value="descent">下降音</option>
                    <option value="electronic">电子音</option>
                    <option value="fanfare">号角声</option>
                    <option value="glass">玻璃声</option>
                    <option value="gotosleep">入睡音</option>
                    <option value="humming">哼唱音</option>
                    <option value="ladder">梯子音</option>
                    <option value="mailsent">邮件发送音</option>
                    <option value="minuet">小步舞曲</option>
                    <option value="newsflash">新闻快报音</option>
                    <option value="noir">黑色音</option>
                    <option value="paymentsuccess">支付成功音</option>
                    <option value="shake">震动音</option>
                    <option value="sherwoodforest">舍伍德森林音</option>
                    <option value="silence">静音</option>
                    <option value="spell">咒语音</option>
                    <option value="suspense">悬疑音</option>
                    <option value="telegraph">电报音</option>
                    <option value="tiptoes">踮脚音</option>
                    <option value="trill">颤音</option>
                    <option value="update">更新音</option>
                  </select>
                  <p class="mt-1 text-sm text-gray-500">选择推送时的音效，留空使用默认音效</p>
                </div>
                <div>
                  <label for="barkBadge" class="block text-sm font-medium text-gray-700">角标数字</label>
                  <input type="number" id="barkBadge" placeholder="1" min="0" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  <p class="mt-1 text-sm text-gray-500">推送角标显示的数字，留空不显示角标</p>
                </div>
                <div>
                  <label for="barkIcon" class="block text-sm font-medium text-gray-700">推送图标 URL (可选)</label>
                  ...
                  <p class="mt-1 text-sm text-gray-500">推送显示的图标，支持 HTTP/HTTPS 链接，留空使用默认图标</p>
                </div>
                <div>
                  <label for="barkGroup" class="block text-sm font-medium text-gray-700">推送分组 (可选)</label>
                  <input type="text" id="barkGroup" placeholder="subscription-reminder" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                  <p class="mt-1 text-sm text-gray-500">推送分组名称，相同分组的推送会归类在一起</p>
                </div>
                <div>
                  <label for="barkUrl" class="block text-sm font-medium text-gray-700">点击跳转 URL (可选)</label>
          ...
          <div class="flex justify-end space-x-4">
            <button type="button" id="resetConfigBtn" class="btn-secondary text-white px-6 py-3 rounded-xl text-sm font-semibold">
              <i class="fas fa-undo mr-2"></i>重置配置
            </button>
            <button type="submit" class="btn-primary text-white px-8 py-3 rounded-xl text-sm font-semibold">
    ...
    <script>
      ...
      document.getElementById('barkTitle').value = config.BARK_TITLE || '{{title}}';
      document.getElementById('barkBody').value = config.BARK_BODY || '{{content}}';
      document.getElementById('barkSound').value = config.BARK_SOUND || '';
      document.getElementById('barkBadge').value = config.BARK_BADGE || '';
      ...
      document.getElementById('configForm').addEventListener('submit', async (e) => {
        ...
        const config = {
          ...
          BARK_TITLE: document.getElementById('barkTitle').value.trim(),
          BARK_BODY: document.getElementById('barkBody').value.trim(),
          BARK_SOUND: document.getElementById('barkSound').value,
          BARK_BADGE: document.getElementById('barkBadge').value.trim(),
      ...
      async function testNotification(type) {
        ...
        if (type === 'telegram') {
          ...
          if (!config.BARK_DEVICE_KEY) {
            showToast('请先填写 Bark 设备 Key', 'warning');
            button.innerHTML = originalContent;
            button.disabled = false;
            return;
          }
      ...
      document.getElementById('resetConfigBtn').addEventListener('click', async () => {
        if (confirm('确定要重置所有配置吗？这将清除所有已保存的设置。')) {
          try {
            ...
            showToast('重置配置失败，请刷新页面重试', 'error');
          }
        }
      });
    ...
    async handleRequest(request, env, ctx) {
      ...
      if (path === '/config') {
        ...
        if (method === 'POST') {
          ...
          BARK_TITLE: newConfig.BARK_TITLE || '{{title}}',
          BARK_BODY: newConfig.BARK_BODY || '{{content}}',
          BARK_SOUND: newConfig.BARK_SOUND || '',
          BARK_BADGE: newConfig.BARK_BADGE || '',
      ...
      if (path === '/test-notification' && method === 'POST') {
        try {
          ...
          if (body.type === 'telegram') {
            ...
            const title = '测试通知';
            const content = '这是一条测试通知，用于验证 Bark 推送功能是否正常工作。\n\n发送时间: ' + formatBeijingTime();
  
            success = await sendBarkNotification(title, content, testConfig);
            message = success ? 'Bark 推送发送成功' : 'Bark 推送发送失败，请检查配置';
          }
  ...
  async function getConfig(env) {
    try {
      ...
      const finalConfig = {
        ...
        BARK_TITLE: config.BARK_TITLE || '{{title}}',
        BARK_BODY: config.BARK_BODY || '{{content}}',
        BARK_SOUND: config.BARK_SOUND || '',
        BARK_BADGE: config.BARK_BADGE || '',
  ...
  async function sendNotificationToAllChannels(title, commonContent, config, logPrefix = '[定时任务]') {
      ...
      if (config.ENABLED_NOTIFIERS.includes('bark')) {
          ...
          console.log(`${logPrefix} 发送Bark推送 ${success ? '成功' : '失败'}`);
      }
  }
  ...
  async function sendBarkNotification(title, content, config) {
    try {
      if (!config.BARK_DEVICE_KEY) {
        console.error('[Bark推送] 通知未配置，缺少设备Key');
        return false;
      }
  
      console.log('[Bark推送] 开始发送通知到设备: ' + config.BARK_DEVICE_KEY);
  
      // 构建推送URL
      const server = config.BARK_SERVER || 'https://api.day.app';
      const deviceKey = config.BARK_DEVICE_KEY;
      
      // 处理标题和内容模板
      let pushTitle = config.BARK_TITLE || '{{title}}';
      let pushBody = config.BARK_BODY || '{{content}}';
      
      // 替换模板变量
      pushTitle = pushTitle
        .replace(/\{\{title\}\}/g, title)
        .replace(/\{\{subscription\}\}/g, '订阅提醒');
      
      pushBody = pushBody
        .replace(/\{\{content\}\}/g, content)
        .replace(/\{\{subscription\}\}/g, '订阅提醒')
        .replace(/\{\{expiryDate\}\}/g, formatBeijingTime(new Date(), 'date'));
  
      // 构建查询参数
      const params = new URLSearchParams();
      params.append('title', pushTitle);
      params.append('body', pushBody);
      
      if (config.BARK_SOUND) {
        params.append('sound', config.BARK_SOUND);
      }
      
      if (config.BARK_BADGE) {
        params.append('badge', config.BARK_BADGE);
      }
      ...
      const pushUrl = `${server}/${deviceKey}?${params.toString()}`;
      console.log('[Bark推送] 推送URL:', pushUrl);
      ...
      const result = await response.text();
      console.log('[Bark推送] 发送结果:', response.status, result);
  
      if (response.ok) {
        try {
          const jsonResult = JSON.parse(result);
          if (jsonResult.code === 200) {
            console.log('[Bark推送] 通知发送成功');
            return true;
          } else {
            console.error('[Bark推送] 发送失败，错误码:', jsonResult.code, '错误信息:', jsonResult.message);
            return false;
          }
        } catch (parseError) {
          // 如果返回的不是JSON，检查是否包含成功标识
          if (result.includes('success') || result.includes('ok')) {
            console.log('[Bark推送] 通知发送成功');
            return true;
          } else {
            console.error('[Bark推送] 解析响应失败:', parseError);
            return false;
          }
        }
      } else {
        console.error('[Bark推送] HTTP请求失败，状态码:', response.status);
        return false;
      }
    } catch (error) {
      console.error('[Bark推送] 发送通知失败:', error);
      return false;
    }
  }
